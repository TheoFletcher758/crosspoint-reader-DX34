...
void CalibreConnectActivity::loop() {
  ...
  if (webServer && webServer->isRunning()) {
    ...
    if (lastHandleClientTime > 0 && timeSinceLastHandleClient > 100) {
      LOG_DBG("CAL", "WARNING: %lu ms gap since last handleClient",
...
void CalibreConnectActivity::render(Activity::RenderLock &&) {
  ...
  if (state == CalibreConnectState::SERVER_STARTING) {
    renderer.drawCenteredText(UI_12_FONT_ID, pageHeight / 2 - 20,
                              tr(STR_CALIBRE_STARTING), true,
                              EpdFontFamily::REGULAR);
  } else if (state == CalibreConnectState::ERROR) {
    renderer.drawCenteredText(UI_12_FONT_ID, pageHeight / 2 - 20,
                              tr(STR_CONNECTION_FAILED), true,
                              EpdFontFamily::REGULAR);
  }
  renderer.displayBuffer();
}
...
void CalibreConnectActivity::renderServerRunning() const {
  ...
  renderer.drawCenteredText(UI_12_FONT_ID, 15, tr(STR_CALIBRE_WIRELESS), true,
                            EpdFontFamily::REGULAR);

  int y = 55 + TOP_PADDING;
  renderer.drawCenteredText(UI_10_FONT_ID, y, tr(STR_WIFI_NETWORKS), true,
                            EpdFontFamily::REGULAR);
  y += LINE_SPACING;
  ...
  renderer.drawCenteredText(
      UI_10_FONT_ID, y + LINE_SPACING,
      (std::string(tr(STR_IP_ADDRESS_PREFIX)) + connectedIP).c_str());

  y += LINE_SPACING * 2 + SECTION_SPACING;
  renderer.drawCenteredText(UI_10_FONT_ID, y, tr(STR_CALIBRE_SETUP), true,
                            EpdFontFamily::REGULAR);
  y += LINE_SPACING;
  ...
  renderer.drawCenteredText(UI_10_FONT_ID, y, tr(STR_CALIBRE_STATUS), true,
                            EpdFontFamily::REGULAR);
  y += LINE_SPACING;
  if (lastProgressTotal > 0 && lastProgressReceived <= lastProgressTotal) {
    ...
    GUI.drawProgressBar(renderer, Rect{barX, y + 22, barWidth, barHeight},
