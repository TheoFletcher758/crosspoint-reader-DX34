...
// begin() and ready() are only called from setup, no need to acquire mutex for
// them
...
String HalStorage::readFile(const char *path) {
  HAL_STORAGE_WRAPPED_CALL(readFile, path);
}
...
size_t HalStorage::readFileToBuffer(const char *path, char *buffer,
  ...
  HAL_STORAGE_WRAPPED_CALL(readFileToBuffer, path, buffer, bufferSize,
                           maxBytes);
}
...
// Allow doing file operations while ensuring thread safety via HalStorage's
// mutex. Please keep the list below in sync with the HalFile.h header
...
bool HalFile::isDirectory() const {
  HAL_FILE_FORWARD_CALL(isDirectory, );
} // already thread-safe, no need to wrap
